...
def _workspace():
    """Lazily construct the Databricks WorkspaceClient so the app can run
    locally without Databricks auth (see lakebase.py)."""
    global _w
    if _w is None:
...
# Locations to fetch weather for by default, separated by ';' (locations
# themselves contain commas, so we can't use comma as the separator).
# NWS only covers the US, so defaults are US cities - non-US locations (e.g.
# "Berlin, Germany") are supported but resolve to no NWS data and are skipped.
# e.g. "Chicago, IL;Austin, TX"
...
def ensure_weather_table():
    ...
    yet. This is the RAW document store the ingest script
    (notebooks/ingest_weather_embeddings.py) reads from to compute vector
    embeddings into the separate `weather_embeddings` table (which is created
    by that script / sql/06_setup_weather_embeddings.sql).
    """
    lakebase.run_write(
        f"""
...
def _weather_embeddings_table_exists() -> bool:
    """Check whether the pgvector embeddings table exists in Lakebase."""
...
def sync_weather_from_nws():
    """
    Harvest unstructured weather text (active alerts, Area Forecast
    Discussions, and gridpoint forecast periods) from the NWS API for a set of
    locations and upsert the normalized documents into `weather_documents` in
    Lakebase.

    Body (optional JSON): {"locations": ["Chicago, IL", "Austin, TX"], "limit": 50}
    Defaults to DEFAULT_WEATHER_LOCATIONS (Chicago + Austin) when no locations
    are supplied. NWS only covers the US, so non-US locations (e.g. Berlin,
    Germany) are supported config but are skipped (0 docs).
    """
    ...
    documents = client.fetch_documents(locations, limit=limit)
...
def search_weather():
    ...
    Embeds the query with the same sentence-transformer used for ingestion
    (loaded once, module-level) and runs a pgvector cosine-similarity search
    (<=> operator) over `weather_embeddings`, joined back to the source
    documents in `weather_documents`.
    """
    ...
    ensure_weather_table()
    if not _weather_embeddings_table_exists():
        return jsonify({"results": [], "message": "No weather embeddings yet - run the ingest script first"})

    query_embedding = embeddings.embed_texts([query.strip()])[0]
    ...
    rows = lakebase.run_query(
        ...
        """,
        (
            embeddings.to_vector_literal(query_embedding),
            embeddings.to_vector_literal(query_embedding),
            top_k,
        ),
    )
    ...
    return jsonify({"query": query.strip(), "top_k": top_k, "results": results})


def _upsert_weather_documents(documents: list[dict]) -> int:
    """Upsert normalized weather documents into Lakebase via ON CONFLICT (id)."""
